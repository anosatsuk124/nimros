...
impl<'a> From<FrameBufferConfig> for FrameBuffer<'a> {
    fn from(value: FrameBufferConfig) -> Self {
        let frame_buffer = unsafe { from_raw_parts_mut(value.frame_buffer, value.size) };
        Self {
            frame_buffer,
...
impl<'a> FrameBuffer<'a> {
    pub fn write_pixel(&mut self, x: usize, y: usize, pixel_color: &PixelColor) {
        let pixel_position = self.pixels_per_scan_line * y + x;
        let base = 4 * pixel_position;
